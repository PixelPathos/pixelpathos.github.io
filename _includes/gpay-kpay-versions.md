...
Expected parameters:
- page.app_name
- include.gpay_store_url
- include.kpay_store_url
- include.kpay_trial_hours
{% endcomment %}

There are two versions of {{ page.app_name }} available in the Connect IQ store. There is no difference in features between the two versions.
...
| **Purchase process** | Payment is required before download. Garmin will send an e-mail once your payment has been processed. {{ page.app_name }} will then download and install automatically the next time your watch syncs. | Download is free. You will then have a {{ include.kpay_trial_hours }}-hour trial. When the trial ends, you will be shown a 6-digit code and URL you can use to make the purchase via the KiezelPay site. Within 5 minutes of completing the purchase, {{ page.app_name }} will unlock. |
